...
from scipy.fft import rfft, rfftfreq, irfft, next_fast_len
...
def main():
    ...
    p1 = nbody.Body(mass = 1.98855E30*u.kg * 1e-5, radius=6371*u.km, colour = "blue", name = "Planet 1")
    p1.keplerian(parent = star, sma = const.au, ecc = 0.1, inc = 0)
    system.addBody(p1)

    # add a super earth-like planet in 1:2 resonance
    resonance = 1/2
    p2 = nbody.Body(mass = 1.98855E30*u.kg * 1e-5, radius=6371*u.km * 4**(1/3), colour = "green", name = "Planet 2")
    p2.keplerian(parent = star, sma = 1.757 * const.au, ecc = 0, inc = 0, arg=0)
    ...
    integration_steps = 200 * 200
    # move up to the next fastest length for fourier fitting
    integration_steps = next_fast_len(integration_steps, real=True)
    ...
    # indexes for the transsits
    idxs = len(transit_full)
    # recover TTV using least squares regression
    A = np.vstack([np.ones(idxs), range(idxs)]).T
    c, m = np.linalg.lstsq(A, transit_full, rcond=-1)[0]

    x_times = range(idxs)

    # find the residuals from the least squares
    residuals = (transit_full - m*np.array(range(idxs))-c)

    # show the found TTV
    plt.plot(range(idxs), residuals)
    plt.show()
    ...
    duration = transit_full[-1] - transit_full[0]
    ...
    # attempt to fit to two sine functions
    def sinfunc(x, a, b, c, d, e, f, g):
        return a * np.sin(b*(x - c)) + d * np.sin(e*(x - f)) + g
    params = (1, 0.5, 0.5, 1, 0.5, 0.5, 1)
    popt, pcov = curve_fit(sinfunc, x_times, residuals, p0=params)
    plt.scatter(x_times,residuals)
    plt.plot(x_times, sinfunc(x_times, *popt), 'r-',label='Fitted function')
    plt.legend()
    plt.show()
